...
namespace Autorizacion.Middleware
{
    public class ClaimsRoles
    {
        ...
        private async Task<IEnumerable<Role>> obtenerInformacionRoles(HttpContext httpContext)
        {
            return await _autorizacionBW.ObtenerRolesXUsuario(new Abstracciones.Modelos.Usuario { NombreUsuario = httpContext.User.Claims.Where(c => c.Type == "usuario").FirstOrDefault().Value });
        }
